...
test.group('#AdminsController - promote admin route', (group) => {
  ...
  group.before(async () => {
    ...
    //id = 2
    await UserFactory.merge({ roleId: 2 }).create()
  ...
  test('An admin must make a regular user another admin', async (assert) => {
    const response = await supertest(BASE_URL)
      .patch('/admins/promote/2')
    ...
    assert.equal('User with id 2 has been promoted', response.text)
    assert.equal(newAdmin.role.role, 'admin')
  })

  test('Must not promote a user if the admin is not authenticated', async (assert) => {
    const response = await supertest(BASE_URL).patch('/admins/promote/2')
  ...
  test('Must not promote a user if the admin is common user', async (assert) => {
    const response = await supertest(BASE_URL)
      .patch('/admins/promote/2')
